...
public partial class VendorInvoiceEditor : ComponentBase
{
    [Inject]
    public IVendorDataService? VendorDataService { get; set; }

    [Inject]
    private ILogger<VendorInvoiceEditor>? Logger { get; set; }
    ...
    private readonly Dictionary<string, object> ReportParameters = new();
    private InvoiceTotals InvoiceTotals { get; set; } = new();
    private string Title { get; set; } = string.Empty;
    private bool Printing = false;
    private const string PostButtonText = "Complete";
    private const string UnpostButtonText = "Revert To Open";
    ...
    {
        if (VendorDataService is null || Invoice is null)
        {
            return;
        }
        ...
        if (Dialogs is not null && !await Dialogs.ConfirmAsync("Mark this invoice as Completed?", "Complete Invoice?"))
        {
            return;
        }
    ...
    {
        if (Invoice is null || Dialogs is null)
        {
            return;
        }
        var saveIt = false;
