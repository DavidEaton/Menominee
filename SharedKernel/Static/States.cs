...
namespace SharedKernel.Static
{
    public static class States
    {
        private static readonly List<State> StatesList = new List<State> {
      //us
      ...
      new State("YT", "Yukon"),
        ...
        {
            return StatesList.Select(s => s.Abbreviation).ToList();
        }

        public static List<string> Names()
        {
            return StatesList.Select(s => s.Name).ToList();
        }

        public static string GetName(string abbreviation)
        {
            return StatesList.Where(s => s.Abbreviation.Equals(abbreviation, StringComparison.CurrentCultureIgnoreCase)).Select(s => s.Name).FirstOrDefault();
        }

        public static string GetAbbreviation(string name)
        {
            return StatesList.Where(s => s.Name.Equals(name, StringComparison.CurrentCultureIgnoreCase)).Select(s => s.Abbreviation).FirstOrDefault();
        }
